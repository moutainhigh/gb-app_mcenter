<%--@elvariable id="command" type="so.wwb.gamebox.model.company.site.vo.SiteConfineIpVo"--%>
...
<form:form id="editForm" action="${root}/sysRole/edit.html" method="post">
    ...
    <div class="modal-body">
        ...
        <div class="form-group clearfix line-hi34 m-b-sm">
            <label class="col-xs-3 al-right">${views.column['SiteConfineIp.startIp']}：</label>
        ...
        <div class="form-group clearfix line-hi34 m-b-sm">
            <label class="col-xs-3 al-right">${views.column['SiteConfineIp.endIp']}：</label>
        ...
        <div class="form-group clearfix line-hi34">
            <label for="result.timeType" class="col-xs-3 al-right">${command.result.type=='2'||command.result.type=='3'?views.column['SiteConfineArea.allow']:views.column['SiteConfineArea.timeType']}：</label>

            <div class="input-group col-xs-8  line-hi34">
                <div class="input-group-btn" >
                    <gb:select name="result.timeType" list="${limitTypeMap}" prompt="" value="${command.result.timeType}"
                               listValue="value" listKey="key" callback="showLimitType"></gb:select>
                ...
                <%--<form:select path="result.timeType" id="timeType" cssClass="btn btn btn-default dropdown-toggle valid" callback="timeTypeChange" >
                    <form:option value="1" dateval="${soulFn:formatDateTz(command.dateList[0], DateFormat.DAY_SECOND,timeZone)}">${dicts.setting.siteConfine['1']}</form:option>
                    <form:option value="2" dateval="${soulFn:formatDateTz(command.dateList[1], DateFormat.DAY_SECOND,timeZone)}">${dicts.setting.siteConfine['2']}</form:option>
                    <form:option value="3" dateval="${soulFn:formatDateTz(command.dateList[2], DateFormat.DAY_SECOND,timeZone)}">${dicts.setting.siteConfine['3']}</form:option>
                    <form:option value="4" dateval="${soulFn:formatDateTz(command.dateList[3], DateFormat.DAY_SECOND,timeZone)}">${dicts.setting.siteConfine['4']}</form:option>
                    <form:option value="5" dateval="${soulFn:formatDateTz(command.dateList[4], DateFormat.DAY_SECOND,timeZone)}">${dicts.setting.siteConfine['5']}</form:option>
                    <form:option value="6" dateval="${soulFn:formatDateTz(command.dateList[5], DateFormat.DAY_SECOND,timeZone)}">${dicts.setting.siteConfine['6']}</form:option>
                    <form:option value="7" dateval="${soulFn:formatDateTz(command.dateList[6], DateFormat.DAY_SECOND,timeZone)}">${dicts.setting.siteConfine['7']}</form:option>
                    <form:option value="8" dateval="">${dicts.setting.siteConfine['8']}</form:option>
                </form:select>--%>
                    </div>
                <span class="stop ${command.result.timeType=='8'?'hide':''}" style="padding-left: 15px">${views.setting_auto['截止']}：</span>
                <c:choose>
        ...
        <div class="clearfix m-b bg-gray p-t-xs">
            ...
            <div class="line-hi25 pull-left col-xs-11 m-b-sm">${views.setting[command.result.type=='1'?'siteConfine.ip.addIpPromptAfter':'siteConfine.ip.addIpPromptAfter.allow']}</div>
        </div>
    </div>
...
<soul:import res="site/setting/siteConfine/ip/Edit"/>
</html>
