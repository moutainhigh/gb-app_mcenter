...
<body>
    ...
    <div class="modal-body">
        <div class="form-group clearfix line-hi34 ${first ? 'hide' : ''}">
            <label for="password" class="col-xs-3 al-right">${views.setting['myAccount.oldPassword']}：</label>
            <div class="col-xs-8 p-x"><input type="password" class="form-control" name="password"></div>
        </div>
        <div class="form-group clearfix line-hi34">
            <label for="newPassword" class="col-xs-3 al-right">${views.setting['myAccount.newPassword']}：</label>
            <div class="col-xs-8 p-x"><input type="password" class="form-control" name="newPassword"></div>
        </div>
        <div class="form-group clearfix line-hi34">
            <label for="newRePassword" class="col-xs-3 al-right">${views.setting['myAccount.newRePassword']}：</label>
            <div class="col-xs-8 p-x"><input type="password" class="form-control" name="newRePassword"></div>
        </div>
    </div>
    <div class="modal-footer">
        <soul:button cssClass="btn btn-filter" text="${views.common.OK}" opType="ajax" precall="validateForm" target="${root}/myAccount/${type == 1 ? 'updatePassword' : 'updatePrivilegePassword'}.html" dataType="json" post="getCurrentFormData" callback="closePage"/>
