...
<c:set var="deal" value='<%= RechargeStatusEnum.DEAL%>' />
        ...
        <div class="col-lg-12">
            <div class="panel panel-default">
                <div class="panel-heading">
                    <h3 class="co-gray6">${views.fund['despoit.check.despoitCheck']}</h3>
                </div>
                <div class="panel-body p-sm">
                    <table class="table no-border table-desc-list">
                        ...
                        <tr>
                            ...
                            <td>
                                <a href="/vPayAccount/detail.html?result.id=${r.payAccountId}&search.type=1" nav-target="mainFrame" class="btn btn-link co-blue">${r.accountType =='2'?r.customBankName:dicts.common.bankname[r.bankCode]}-${r.fullName}</a>
                        ...
                        <tr>
                            ...
                            <td class="money">
                                    ${r.currencySign}${soulFn:formatInteger(r.rechargeTotalAmount)}<i>${soulFn:formatDecimals(r.rechargeTotalAmount)}</i>
                                <c:choose>
                                ...
                                <c:if test="${r.favorableTotalAmount>0}">
									...
									<span class="co-gray9 m-l-md">
										...
										<c:if test="${empty command.activityAudit||!command.activityAudit}">
                                            ...
                                            <a style="margin-left: 10px;" href="/operation/activityType/viewActivityDetail.html?search.id=${command.activityId}" title="${command.activityName}" nav-target="mainFrame">${views.fund_auto['查看优惠活动']}</a>
                                        </c:when>
                                        <c:when test="${r.rechargeStatus=='2'}">
                                            <c:if test="${command.activityStatus=='2'}">
                                                <span class="co-green">[${views.common['checkPass']}]</span>
                                            </c:if>
                                            <c:if test="${command.activityStatus=='6'}">
									...
									<span class="co-gray9 m-l-md">
										...
										<c:if test="${command.activityStatus=='1'}">
                                            <a style="margin-left:10px" href="/operation/vActivityPlayerApply/activityPlayerApply.html?search.activityMessageId=${command.activityId}&search.playerId=${r.playerId}&search.playerRechargeId=${r.id}" nav-target="mainFrame">${views.fund_auto['去审核']}</a>
                                        </c:if>
										<c:if test="${command.activityStatus=='2'}">
                                            <a style="margin-left:10px" href="/report/vPlayerFundsRecord/view.html?search.id=${command.favorableTransactionId}" nav-target="mainFrame">${views.fund_auto['查看优惠记录']}</a>
                                        </c:if>
									</span>
                                        </c:when>
                                    </c:choose>
                        ...
                        <tr class="${r.rechargeStatus=='1'?'warning':''}${r.rechargeStatus=='2'?'success':''}${r.rechargeStatus=='3'?'danger':''} major">
                            ...
                            <td class="money">
                                ...
                                <span class="${r.rechargeStatus=='2'?'co-green':''}${r.rechargeStatus=='1'?'co-orange':''}${r.rechargeStatus=='3'?'co-red':''}">[${dicts.fund.recharge_status[r.rechargeStatus]}]</span>
                                <c:if test="${r.rechargeStatus=='1'}">
                                    <soul:button permission="fund:companydeposit_check" target="confirmCheckPass" data="${r.id}" opType="function" tag="button" text="${views.common['checkPass']}" cssClass="btn btn-primary p-x-sm m-l-sm"/>
                                    <soul:button permission="fund:companydeposit_check" target="checkFailure" data="${r.id}" opType="function" text="${views.common['checkFailure']}" cssClass="btn btn-danger p-x-sm m-l-sm" tag="button"/>
                                </c:if>
                        ...
                        <tr>
                            ...
                            <td>
                                <textarea class="form-control width-response" rows="4" maxlength="200" id="result.checkRemark" name="result.checkRemark">${r.checkRemark}</textarea>
                        ...
                        <tr class="active">
                            ...
                            <td>
                                <div class="btn-groups text-left">
                                    <soul:button target="next" data="${r.id}" text="${views.fund['despoit.index.next']}" opType="function" cssClass="btn btn-primary-hide p-x-lg">
                                        <span class="fa fa-arrow-right"></span>${views.fund['despoit.index.next']}
                                    </soul:button>
                                </div>
                            </td>
                        </tr>
...
<soul:import res="site/fund/deposit/company/Check"/>