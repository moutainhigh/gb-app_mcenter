...
<script id="VPlayerWithdrawListVo" type="text/x-jsrender">
    ...
    <%--<soul:button target="${root}/fund/withdraw/payment.html?search.transactionNo={{:transactionNo}}" callback="query" confirm="${views.fund_auto['确认出款？']}" cssClass="label label-info p-x-md" text="${views.fund_auto['出款']}" opType="ajax" />--%>
                        ...
                        {{if checkStatus=='payment_success'}}
                            <soul:button target="withdrawStatusView" dataId="{{:id}}" size="auditLogCss" callback="" cssClass="label label-success p-x-md" text="${views.fund_auto['出款成功']}" opType="function" />
                        {{/if}}
                        {{if checkStatus=='payment_fail'}}
                            <%--<shiro:hasPermission name="fund:withdraw_payment">--%>
                                <soul:button target="withdrawStatusView" dataId="{{:id}}" size="auditLogCss" callback="" cssClass="label label-danger p-x-md" text="${views.fund_auto['出款失败']}" opType="function" />
                            <%--</shiro:hasPermission>--%>
                            <%--<shiro:lacksPermission name="fund:withdraw_payment">--%>
