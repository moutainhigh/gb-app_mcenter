...
<html>
<head>
    <title>${views.fund['withdraw.index.auditFail']}</title>
...
<form:form action="${root}/fund/withdraw/withdrawList.html" method="post">
    ...
    <c:if test="${fn:length(failReasons)>0}">
        ...
        <div class="modal-footer">
            <soul:button cssClass="btn btn-warning btn-withdraw-result-btn" tag="button" text="${views.common['confirmFailure']}" opType="function" target="withdrawFailure" callback="saveCallbak"/>
            <soul:button target="closePage" text="${views.common['cancel']}" cssClass="btn btn-default" opType="function" callback="saveCallbak"/>
        </div>
    </c:if>
    <c:if test="${fn:length(failReasons)<=0}">
