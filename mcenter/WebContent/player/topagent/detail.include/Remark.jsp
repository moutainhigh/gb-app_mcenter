...
<!--备注-->
<form:form id="topagentRemarkForm" action="${root}/playerRemark/topagentRemark.html?search.entityUserId=${command.search.entityUserId}&search.operatorId=${command.search.operatorId}" method="post">
    <div class="clearfix remark-wrap">
        <soul:button target="${root}/playerRemark/create.html?result.entityUserId=${command.search.entityUserId}&result.remarkType=remark&result.model=agent&result.entityId=${command.search.entityUserId}" opType="dialog" callback="remark.callBack" cssClass="btn btn-info btn-addon m-l-xs pull-left" text="${views.role['player.view.remark.addRemark']}">
            <i class="fa fa-plus"></i>${views.role['player.view.remark.addRemark']}
        </soul:button>
            <%--
            <c:if test="${listVo.paging.totalCount>1}">
                <soul:button target="${root}/playerRemark/moreRemark.html?search.playerId=${listVo.search.playerId}" opType="dialog" cssClass="pull-right co-blue" callback="remark.queryRemark" text="${views.role['player.view.remark.viewMore']} &gt;&gt;"/>
            </c:if>
            --%>
    </div>
    <br>
...
<script>
    curl(["site/player/topagent/Remark"], function (Remark) {
