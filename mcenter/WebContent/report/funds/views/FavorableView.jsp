...
<input type="hidden" name="usernames" value="${command.result.username}" />
<div class="col-lg-12">
    <div class="panel panel-default">
        <div class="panel-heading">
            <h3 class="co-gray6">${views.report['fund.detail.favorable']}</h3>
        </div>
        <div class="panel-body p-sm">
            <table class="table no-border table-desc-list">
                ...
                <tr>
                    ...
                    <td>
                        <a href="/player/playerView.html?search.id=${command.result.playerId}" nav-target="mainFrame" class="btn btn-link co-blue">${command.result.username}</a>
                ...
                <tr>
                    ...
                    <td>${dicts.common.transaction_way[command.result.transactionWay]}</td>
                </tr>
                <tr>
                    <th scope="row" class="text-right">${views.report_auto['优惠名称']}：</th>
                    <td>
                        <a <c:choose>
                                <c:when test="${empty command.activityMessageId}">
                                    href="javascript:void(0);"
                                </c:when>
                                <c:otherwise>
                                    href="/operation/activityType/viewActivityDetail.html?search.id=${command.activityMessageId}" nav-target="mainFrame"
                                </c:otherwise>
                            </c:choose>>
                        ${command.result.transactionData}
                            <span class="co-grayc2">
                                <c:choose>
                                    <c:when test="${empty command.activityMessageId}">
                                        ${views.report_auto['该活动已被删除']}
                                    </c:when>
                                </c:choose>
                            </span>
                        </a>
                        ...
                        <c:choose>
                            ...
                            <c:otherwise>
                                ...
                                <c:if test="${command.favorableSource eq 'activity_favorable' or command.favorableSource eq 'backwater_favorable'}">
                                  ...
                                  ${command.auditFavorableMultiple}倍
                            </c:otherwise>
                        </c:choose>
                    </td>
                </tr>
                ...
                <tr class="${tr_class}">
                    <th scope="row" class="text-right">${views.report_auto['优惠金额']}：</th>
                    <td class="money">
