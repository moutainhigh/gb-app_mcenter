                ...
                <div class="tab-content">
                    <div class="table-responsive">
                        <table class="table table-striped table-hover dataTable m-b-none" aria-describedby="editable_info">
                            ...
                            <tbody>
                            <tr>
                                <c:if test="${sameDay}"><td>${soulFn:formatNumber(command.operate.playerNum)}</td></c:if>
                                <td>${soulFn:formatNumber(command.operate.transactionOrder)}</td>
                                <td>${soulFn:formatCurrency(command.operate.transactionVolume)}</td>
                                <td>${soulFn:formatCurrency(command.operate.effectiveTransaction)}</td>
                                <td>${soulFn:formatCurrency(command.operate.profitLoss)}</td>
                            </tr>
                            </tbody>
                        </table>
                    </div>
                ...
                <div class="tab-content">
                    <div class="table-responsive">
                        <table class="table table-striped table-hover dataTable m-b-none" aria-describedby="editable_info">
                            ...
                            <tr>
                                <th>${views.report['operate.detail.account.site']}</th>
                            ...
                            <c:forEach var="res" items="${command.result}">
                                <tr>
                                    <input type="hidden" name="siteId" value="${res.siteId}"/>
                                    <td><soul:button target="query" opType="function" precall="detailSite" text="${gbFn:getSiteName(res.siteId)}" /></td>
                                    ...
                                    <td><soul:button target="query" opType="function" precall="detailSite" text="${views.common['detail']}" /></td>
                                </tr>
                            </c:forEach>
                            </tbody>
                        </table>
