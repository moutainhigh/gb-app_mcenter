...
<html>
    ...
    <body>
        <form:form action="${root}/player/playerView.html?search.id=${command.result.id}">
            <div class="modal-body">
                ...
                <%--貌似没用--%>
                <%--<div class="form-group clearfix m-b-sm hide">
                    <label class="col-xs-3 al-right">${views.share_auto['账号冻结']}：</label>
                    <div class="col-xs-9">
                        <input type="checkbox" name="my-checkbox" disabled data-size="mini" >
                    </div>
                </div>--%>
                <div class="form-group clearfix m-b-sm">
                    ...
                    <div class="col-xs-9">
                        <div class="input-group">
                            <span class="input-group-addon bdn">${views.share_auto['开始时间']}：${soulFn:formatDateTz(command.result.balanceFreezeStartTime, DateFormat.DAY_SECOND,timeZone)}</span>
                            <span class="input-group-addon bdn">${views.share_auto['截止时间']}：${soulFn:formatDateTz(command.result.balanceFreezeEndTime, DateFormat.DAY_SECOND,timeZone)}</span>
                        </div>
                    </div>
                </div>
                <c:if test="${not empty command.result.balanceFreezeTitle}">
                    <div class="form-group clearfix m-b-sm line-hi34">
                        <label class="col-xs-3 al-right">${views.share_auto['冻结原因']}：</label>
                        <div class="col-xs-9">
                            ${command.result.balanceFreezeTitle}
                        </div>
                    </div>
                </c:if>
                <c:if test="${not empty command.result.balanceFreezeContent}">
                    <div class="form-group clearfix m-b-sm line-hi34">
                        <label class="col-xs-3 al-right">${views.share_auto['内容']}：</label>
                        <div class="col-xs-9">
                                ${command.result.balanceFreezeContent}
                        </div>
                    </div>
                </c:if>
                <div class="form-group clearfix m-b-sm">
                    ...
                    <div class="col-xs-9">
                        <div class="gray-chunk clearfix">${command.result.balanceFreezeRemark}</div>
                    </div>
                </div>
            ...
            <div class="modal-footer">
                <soul:button target="${root}/share/account/cancelBalanceFreeze.html?search.id=${command.result.id}" tag="button" cssClass="btn btn-filter" callback="saveCallbak" text="" opType="ajax">
                    ${views.common['commit']}
                </soul:button>
