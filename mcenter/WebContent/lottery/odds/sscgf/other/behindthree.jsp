        ...
        <div class="tab-content">
            <div class="table-responsive">
                <table class="table table-striped table-bordered table-hover dataTable m-b-none text-center" aria-describedby="editable_info">
                    ...
                    <tbody>
                    <tr>
                        <th><span>后三和值尾数</span></th>
                        <c:forEach items="${command.ssc_sanxing_zuxuan_hshzws}" var="p" >
                            <td>
                        ...
                        <c:forEach items="${command.ssc_sanxing_zuxuan_hsts}" var="p" >
                             <c:if test="${p.betNum eq '豹子'}">
                                 <th><span>后三特殊号(豹子)</span></th>
                                  <td>
                                  <div class="input-group content-width-limit-10">
                                            <c:set var="odd" value="${p}"/>
                                            <input type="hidden" value="${odd.id}" name="lotteryOdds[${status.index}].id">
                                ...
                                <c:if test="${p.betNum eq '顺子'}">
                                    <th><span>后三和值尾数(顺子)</span></th>
                                    <td>
                                        <div class="input-group content-width-limit-10">
                                            <c:set var="odd" value="${p}"/>
                                            <input type="hidden" value="${odd.id}" name="lotteryOdds[${status.index}].id">
                    ...
                    <tr>
                        <c:forEach items="${command.ssc_sanxing_zuxuan_hsts}" var="p" >
                                <c:if test="${p.betNum eq '对子'}">
                                    <th><span>后三和值尾数(对子)</span></th>
                                    <td>
                                        <div class="input-group content-width-limit-10">
                                            <c:set var="odd" value="${p}"/>
                                            <input type="hidden" value="${odd.id}" name="lotteryOdds[${status.index}].id">
