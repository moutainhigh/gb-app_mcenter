<%@ page import="so.wwb.gamebox.model.report.operation.po.StationBill" %>
<%--@elvariable id="command" type="so.wwb.gamebox.model.report.operation.vo.StationBillListVo"--%>
<%@page contentType="text/html;charset=UTF-8" language="java" pageEncoding="UTF-8" %>
<%@ include file="/include/include.inc.jsp" %>
<c:set var="poType" value="<%= StationBill.class %>"></c:set>
<!--//region your codes 1-->
    ...
    <table class="table table-striped table-hover dataTable" aria-describedby="editable_info">
        <thead>
            <tr class="bg-gray">
                ...
                <th>${views.column['StationBill.topAgent']}</th>
                <th>${views.column['StationBill.billNum']}</th>
                <th>${views.column['StationBill.billName']}</th>
                <soul:orderColumn poType="${poType}" property="amountPayable" column="${views.column['StationBill.amountPayable_1']}"/>
        ...
        <c:forEach items="${command.result}" var="p" varStatus="status">
            <tr>
                ...
                <td>
                    <a href="/operation/stationbill/generalBillView.html?search.id=${p.id}" nav-target="mainFrame">${views.common['detail']}</a>
                </td>
            </tr>
        </c:forEach>
        </tbody>
