                ...
                <table class="table border" id="backWaterTable">
                    ...
                    <c:forEach items="${rakebackSets}" var="r">
                        <tr>
                            ...
                            <td>
                                <c:if test="${r.status eq 0}"><span class="label label-danger">${views.common['disabled']}</span></c:if>
                                <c:if test="${r.status eq 1}"><span class="label label-success">${views.common['normal']}</span></c:if>
                                <c:if test="${r.status eq 2}"><span class="label label-warning">${views.common['delete']}</span></c:if>
                            </td>
                        </tr>
                    </c:forEach>
                </table>
