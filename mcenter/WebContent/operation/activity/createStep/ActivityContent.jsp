        ...
        <div class="wrapper white-bg shadow">
            ...
            <div class="clearfix m-t-sm line-hi34">
                ...
                <div class="col-sm-5">
                    <c:if test="${activityType.result.code ne 'money'}">
                        <label><input type="radio" value="true" name="activityMessage.isDisplay" class="i-checks" ${(empty activityMessageVo.result || activityMessageVo.result.isDisplay) eq 'true'? "checked":""}>${views.operation['Activity.step.isDisplay.true']}</label>
                        <label><input type="radio" value="false" name="activityMessage.isDisplay" class="i-checks" ${activityMessageVo.result.isDisplay eq 'false'? "checked":""}>${views.operation['Activity.step.isDisplay.false']}</label>

                    </c:if>
                    <c:if test="${activityType.result.code eq 'money'}">
                        <label><input type="radio" value="true" name="activityMessage.isDisplay" class="i-checks" ${(activityMessageVo.result.isDisplay) eq 'true'? "checked":""}>${views.operation['Activity.step.isDisplay.true']}</label>
                        <label><input type="radio" value="false" name="activityMessage.isDisplay" class="i-checks" ${empty activityMessageVo.result || activityMessageVo.result.isDisplay eq 'false'? "checked":""}>${views.operation['Activity.step.isDisplay.false']}</label>
            ...
            <div class="clearfix m-t-md">
                ...
                <div class="col-sm-5">
                    <gb:dateRange format="${DateFormat.DAY_SECOND}" style="width:160px" useRange="true"
                                  startDate="${activityMessageVo.result.startTime}" endDate="${activityMessageVo.result.endTime}"
                                  startName="activityMessage.startTime" endName="activityMessage.endTime"></gb:dateRange>
                </div>
            ...
            <c:if test="${activityType.result.code eq 'content'}">
                ...
                <div class="clearfix m-t-md line-hi34">
                    ...
                    <span class="col-sm-5"><input type="checkbox" class="i-checks" id="levels" name="activityRule.isAllRank" ${activityRule.isAllRank ?"checked":""}/>${views.operation['Activity.step.allRank']}</span>
                        ...
                        <ul class="nav nav-tabs">
                            ...
                            <c:forEach var="siteLang" items="${languageList}" varStatus="index">
                                <li class=" ${index.index==0?'active':''}">
                                    <a id="a_${index.index}" data-toggle="tab" href="#tab${index.index}" aria-expanded="${index.index==0?'true':'false'}">
                                            ${fn:substringBefore(dicts.common.language[siteLang.value.language], '#')}
                                        <span class="_editStatus${index.index}">
                                            <c:choose>
                                                <c:when test="${(not empty activityMessageI18ns[siteLang.value.language].activityAffiliated) and (not empty activityMessageI18ns[siteLang.value.language].activityName) and (not empty activityMessageI18ns[siteLang.value.language].activityDescription)}">
                                                    ${views.common['edited']}
                                                </c:when>
                                                <c:otherwise>
                                ...
                                <input type="hidden" name="activityMessageI18ns[${index.index}].activityVersion" value="${siteLang.value.language}">
                            </c:forEach>
                        </ul>
                    </div>
                </div>
                        ...
                        <c:forEach var="siteLang" items="${languageList}" varStatus="index">
                            <div id="tab${index.index}" class="tab-pane ${index.index==0?'active':''}">
                                ...
                                <div class="clearfix m-t-md" id="secondary${index.index}">
                                    ...
                                    <div class="col-sm-5">
                                        ...
                                        <div id="activityAffiliatedImg${index.index}">
                                            <img id="dd_${index.index}" src="" style="display: none;width:100%;height: auto;"/>
                                        </div>
                                        ...
                                        <div class="form-group m-b-sm">
                                            <div id="activityContentImage${index.index}">
                                                <c:if test="${not empty activityMessageI18ns[siteLang.value.language].activityCover}">
                                                    <img id="bb_${index.index}" src="${soulFn:getThumbPath(domain, activityMessageI18ns[siteLang.value.language].activityCover,500,500)}" class="logo-size-h100" style="margin: 10px 0; width: auto;height: 250px;"/>
                                                </c:if>
                                            </div>
                                            <input id="activityContentFile" bbb="${index.index}" class="file file1" type="file" target="activityMessageI18ns[${index.index}].activityCover" accept="image/*" name="activityCoverImg">
                                            <input type="hidden" class="activityContentFile" bbb="${index.index}" name="activityMessageI18ns[${index.index}].activityCover" value="${activityMessageI18ns[siteLang.value.language].activityCover}">
                                            <input type="hidden" value="${activityMessageI18ns[siteLang.value.language].activityCover}">
                                        </div>
                                        <div id="activityContentImg${index.index}">
                                            <img id="aa_${index.index}" src="" style="display: none;width: 100%;height: auto;"/>
                                        </div>
                                ...
                                <div class="clearfix m-t-md">
                                    ...
                                    <div class="col-sm-9">
                                        <textarea class="form-control activityOverview" placeholder="${views.operation['Activity.step.message3']}" bbb="${index.index}" name="activityMessageI18ns[${index.index}].activityOverview" id="activityOverview${index.index}">${activityMessageI18ns[siteLang.value.language].activityOverview}</textarea>
                                    </div>
                                </div>--%>
                                ...
                                <div class="clearfix m-t-md">
                                    ...
                                    <div class="col-sm-9">
                                        <textarea class="_editArea${index.index} contents" bbb="${index.index}" id="editContent${index.index}" ueditorId="editContent${index.index}" name="activityMessageI18ns[${index.index}].activityDescription">${activityMessageI18ns[siteLang.value.language].activityDescription}</textarea>
                                    </div>
                                </div>
                            </div>
                        </c:forEach>
            ...
            <div class="operate-btn">
                <c:if test="${!(activityMessageVo.result.checkStatus eq '1' && (activityMessageVo.states eq 'processing' || activityMessageVo.states eq 'notStarted'))  || activityMessageVo.result.checkStatus eq '2'}">
                    <soul:button callback="getActivityMessageId" precall="uploadFile" target="${root}/operation/activity/activityContentDraft.html?activityState=draft" text="${views.operation['Activity.step.saveAndDraft']}" opType="ajax" cssClass="btn btn-filter btn-lg" post="getCurrentFormData"/>
                </c:if>
                <c:choose>
                    <c:when test="${activityType.result.code eq 'content'}">
                        <soul:button precall="validateForm" opType="function" target="activityContentTypeNext" cssClass="btn btn-filter btn-lg" text="${views.common['next']}" />
                    </c:when>
                    <c:otherwise>
                        <soul:button precall="validateForm" opType="function" target="activityContentNext" cssClass="btn btn-filter btn-lg" text="${views.common['next']}" />
                    </c:otherwise>
                </c:choose>
            </div>
        </div>
