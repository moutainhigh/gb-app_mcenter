    ...
    <table class="table table-striped table-hover dataTable" aria-describedby="editable_info">
        ...
        <c:forEach items="${command.result}" var="s" varStatus="status">
            ...
            <tr>
                ...
                <td title="${s.replyTitle}">
                    <c:choose>
                        <c:when test="${!s.read}">
                            <b>${fn:substring(s.replyTitle, 0, 30)}<c:if test="${fn:length(s.replyTitle)>30}">...</c:if></b>
                        </c:when>
                        <c:otherwise>
                            ${fn:substring(s.replyTitle, 0, 30)}<c:if test="${fn:length(s.replyTitle)>30}">...</c:if>
                        </c:otherwise>
                    </c:choose>
                </td>
                <td>
