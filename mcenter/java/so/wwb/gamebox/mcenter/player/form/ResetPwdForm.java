...
public class ResetPwdForm implements IForm {
    ...
    @Compare(logic = CompareLogic.EQ, anotherProperty = "password",message = "common.两次输入的密码不一致")
    @Depends(property = "$resetType", operator = Operator.EQ,value ="loginPwd")
    ...
    public String get$informType() {
        return $informType;
    }

    public void set$informType(String $informType) {
        this.$informType = $informType;
    }
    @Depends(property = "$informType",operator = Operator.EQ,value = "true", jsValueExp = "document.getElementById(\"mailCheck\").checked==true")
    ...
    @Compare(logic = CompareLogic.EQ, anotherProperty = "permissionPwd",message = "common.两次输入的密码不一致!")
    @Depends(property = "$resetType", operator = Operator.EQ,value ="payPwd")
